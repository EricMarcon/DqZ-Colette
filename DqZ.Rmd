...
Le terme *diversité* doit être réservé aux nombres de Hill [@Jost2006], l'entropie doit être appelée entropie est le terme "indice" évité.
...
> Dans l'espace engendré par cette matrice de distance :
```{r}
library("ade4")
D %>% 
  as.dist %>% 
...
La banalité de chaque pixel est calculée comme la similarité moyenne avec les autres, sa rareté est l'inverse de sa banalité et l'entropie de l'image est la banalité moyenne des pixels.
...
Le paramètre $u$ fixe la vitesse de la décroissance de la similarité en fonction de la distance.
Son choix est libre: on peut chercher la valeur qui optimisera la corrélation entre la diversité des pixels et celle des arbres.
